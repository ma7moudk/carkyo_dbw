...
def readSerial(event):
    ...
    if (ard.inWaiting() > 0):
        ...
        if len(frame) == FRAME_LENGTH:
            ...
            if ( ord(chr(frame[0])) == 255 and ord(chr(frame[1])) == 255 and ord(chr(frame[2])) <= 3): #2,3,4,5
                pot_val = ord(chr(frame[2]))*256 + ord(chr(frame[3]))
                rf_state = chr(ord(chr(frame[4])))
                received_angle = ord(chr(frame[5]))*256 #TODO
                for i in range(FRAME_LENGTH): #clearing the frame after reading
                    frame.popleft()
                    
                reading_done = 1

            elif ( ord(chr(frame[4])) == 255 and ord(chr(frame[5])) == 255 and ord(chr(frame[0])) <= 3): #0,1,2,3
                pot_val = ord(frame[0])*256 + ord(frame[1])
                rf_state = chr(ord(chr(frame[2])))
                received_angle = ord(chr(frame[3]))*256 #TODO
                for i in range(FRAME_LENGTH - 2): #clearing the frame after reading
                    frame.popleft()
                    
                reading_done = 1
              
            elif ( ord(chr(frame[5])) == 255 and ord(chr(frame[0])) == 255 and ord(chr(frame[1])) <= 3): #1,2,3,4
                pot_val = ord(frame[1])*256 + ord(frame[2])
                rf_state = chr(ord(chr(frame[3])))
                received_angle = ord(chr(frame[4]))*256 #TODO
