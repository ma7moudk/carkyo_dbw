...
def brake(brakeVal):
    ...
    GPIO.output(brakePins, intValues)

last_acc = 0
def accelerate(accVal):
    global accSwitch, last_acc
    ...
    # To convert from (0->64) to (0->4096) myltiply by (4096/64) = 64 (Equation was calculated using power 3.3)
    # So when using power 5 myltiply by (64 * 3.3 / 5.0) = 42.24
...
def callback_speed_feedback(data):
    ...
    if (rospy.Time.now()-last_command_time>=rospy.Duration(1.0)):
        ...
        logLinePrint = "speed: %2.3f, pid: %2.3f, dac: %2.1f, vmps3: %2.3f, vr: %2.3f, vl: %2.3f, angNeededCh: %2.3f, error: %2.3f, brake: %2.1f\n" %(speed, PIDspeed, DACvalue, mean_Vmps, vR, vL, needed_angle_change, error, last_brake)
        logLine = "%2.3f,%2.3f,%2.1f,%2.3f,%2.3f,%2.3f,%2.3f,%2.3f,%2.1f\n" %(speed, PIDspeed, DACvalue, mean_Vmps, vR, vL, needed_angle_change, error, last_brake)
        print(logLinePrint)  
...
# Listener
def listener():
    rospy.Subscriber("/carkyo/command", AckermannDriveStamped, callback_move_base, queue_size=2)
